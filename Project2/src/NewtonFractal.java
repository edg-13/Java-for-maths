...
class NewtonFractal {
    ...
    public int findRoot(Complex root) {
        ...
        return (i==roots.size()) ? -1 : i; //ternary operator
    }
    ...
    public Complex pixelToComplex(int i, int j) {
        // You need to fill in this function.
        double x = origin.getReal() + i*(width/NUMPIXELS);
        double y = origin.getImag() - j*(width/NUMPIXELS);
    ...
    public void createFractal(boolean colorIterations) {
        ...
        for(int i = 0; i<NUMPIXELS; i++) {
            for(int j = 0; j<NUMPIXELS; j++) {
                Complex a = pixelToComplex(i,j);
                iterator.iterate(a); //or make new object for each (i,j)?
                if (findRoot(iterator.getRoot())==-1) {
                    roots.add(iterator.getRoot());
                }
                //System.out.println(iterator.getNumIterations());
    ...
    public static void main(String[] args) {
        ...
        Complex[] coeff = new Complex[] { new Complex(1.0,0.0), new Complex(),
                new Complex(-2, 0), new Complex(1.0,0.0), new Complex(1,0), new Complex(1,0) };
