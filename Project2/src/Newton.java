...
class Newton {
    ...
    public void iterate(Complex z0) {
        // You need to fill in this method.
        numIterations = 0;
        Newton g = new Newton(f);
        Polynomial d = g.getFp();
        while (numIterations<MAXITER) {
            if (fp.evaluate(z0).getReal()==0 && fp.evaluate(z0).getImag()==0) {
                err=-1;
                numIterations++;
                break;
            }
            else {
                if (((f.evaluate((z0))).divide(fp.evaluate(z0))).abs()<TOL) {
                    //System.out.println(z0.add(((f.evaluate(z0)).divide(fp.evaluate(z0))).minus()));
                    err = 0;
                    root = z0.add(((f.evaluate(z0)).divide(fp.evaluate(z0))).minus());
    ...
    public static void main(String[] args) {
        ...
        System.out.println((a.add(n.root.minus())).abs()<Newton.TOL);
